...
class MyStrategy(StrategyTemplete1):
    ...
    def scenarioHistF2(self, longRef, shortRef, scenarioName, num):
        ...
        longSort = seriesTop(scenarioSummary[u'sharpe'][scenarioName], 5, ascending=False)[0]  # 长期
    ...
    def onBar(self):
        ...
        if self.getLoc() == self.getPeriodrange(freq=u'M')[0] and curMonth in ['01', '07']:           # 按半年调仓
            ...
            for targetIndex in targetIndexList:
                ...
                seedFund = seedFund.append(pd.Series([dividedSeedfund/num1]*num1, index=targetIndex))   # 标的金额-等权
...
def aTestCase(primaryDate, startTime, endTime):
    ...
    indexNameList = []
    stockCodelist = []
    strategyNamelist = [u"Manager's sharpe FOF策略_半年"]
    tester = u'李琦杰'
    testContent = u'不采用情景分析, 标的等权, 复利'
