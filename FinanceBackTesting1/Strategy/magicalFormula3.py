...
class MyStrategy(StrategyTemplete1):
    ...
    def onBar(self):
        ...
        # if curDate == '2015-05-08':
        #     pass
        print(curDate)
        month = self.getPeriod(freq=u'M').split('-')[-1]
        direction = 1

        grossprofitmargin = self.grossprofitmargin.loc[self.getTime(ref=1)].dropna().sort_values(ascending=False)[:200]     # 大值排前
        roa = self.roe.loc[self.getTime(ref=1)].dropna()
        roa = roa[roa < 100].loc[grossprofitmargin.index].sort_values(ascending=False)[:100]    # 大值排前

        # roe = self.roe.loc[self.getTime(ref=1)].loc[roa.index]
        # roe = roe[roa > 0].dropna().sort_values(ascending=False)[:200]
        rankTarget1 = roa
        peg = self.peg.loc[self.getTime(ref=1)][rankTarget1.index]
        rankTarget = peg[peg > 0.8]
        print("rankTarget %s " % rankTarget)
        # rankTarget = self.earningYield.loc[self.getTime(ref=1)].loc[rankTarget1.index]
        yzb = self.index(u'yiziban', u'stocks')
        yzb = yzb[yzb == 1].index
        newStock = self.index(u'is_new_stock_504', u'stocks').index
        tradeIndex = self.cleanedDf().loc[curDate].dropna().index

        targetIndex = rankTarget[rankTarget.index.difference(yzb | newStock) & tradeIndex].sort_values(ascending=True)[:5].index
