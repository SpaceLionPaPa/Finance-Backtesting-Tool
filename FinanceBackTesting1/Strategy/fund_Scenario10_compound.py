...
class MyStrategy(StrategyTemplete1):
    ...
    def scenarioHistF(self, st, en, scenarioName, num):
        ...
        distDict[u'sharpe'][scenarioName] = seriesTop(scenarioSummary[u'sharpe'][scenarioName], 5, ascending=False)[0]
...
def aTestCase(primaryDate, startTime, endTime):
    ...
    indexNameList = []
    stockCodelist = []
    strategyNamelist = [u'市道夏普率FOF策略_复利']
    tester = u'李琦杰'
    ...
    # data2 = pd.read_csv(u'../Data/Main/股票型基金日期.csv', index_col=0)[u'最新净值日期']
    # data3 = pd.read_csv(u'../Data/Main/债券型基金日期.csv', index_col=0)[u'最新净值日期']
    # fundLatestRptDate = data1.append(data2).append(data3)
