...
class StrategyTemplete1(object):
    ...
    def __profitAndLoss(self):
        ...
        PL = 0.
        # 不同方向
        if self.TradeTime in ['2016-01-04']:
            pass
    ...
    def sendOrder(self, sendOrderDict):
        ...
        if action == -1:                                        # 平仓：手续费+平仓盈亏
            amtChg = costFee + self.__closingPL(sendOrderDict)
        else:
            amtChg = costFee                                    # 开仓：只计算手续费
        ...
        self.histRecord[u'orderRecord'][self.TradeTime][orderNum][u'amount after cost'] = amt - abs(costFee)
    ...
    def smartSendOrder(self, orderRecord, priceMode, volMode, *arg):
        ...
        if orderRecord:
            ...
            for i in range(orderLen):                                   # 按单号正序进行模拟交易
                ...
                price = price * (1 + int(action) * impactCostRatio)
                ...
                orderDict[u'dataTarget_Price'] = price
